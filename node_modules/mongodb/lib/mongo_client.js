 ...
 *   const models = client.models(dbName);
 *   client.close();
 * });
 *
 * @example
...
/**
 * Close the models and its underlying connections
 * @method
...
/**
 * Create a new Db instance sharing the current socket connections. Be aware that the new models instances are
 * related in a parent-child relationship to the original instance so that events are correctly emitted on child
 * models instances. Child models instances are cached so performing models('db1') twice will return the same instance.
 ...
 * @param {object} [options] Optional settings.
 * @param {boolean} [options.noListener=false] Do not make the models an event listener to the original connection.
...
MongoClient.prototype.db = function(dbName, options) {
  ...
  // Default to models from connection string if not provided
  if (!dbName) {
  ...
  // Do we have the models in the cache already
  ...
  if (!this.topology) {
    throw new MongoError('MongoClient must be connected before calling MongoClient.prototype.models');
  }
